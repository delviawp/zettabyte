...
export class NtcComponent implements OnInit {
  ...
  addToCart(item: Item ) {
    ...
    addCart(this.cart, item);
    this.cart.forEach((e) => this.totalCart += e.price)
  } 

  BuyButton() {
    if (this.totalCart <= 1000 && this.cart.length > 0){
        Swal.fire({
          position: 'center',
          icon: 'info',
...
function addCart (arr: Item[], obj: Item) {
  const i = arr.findIndex((e) => e.name === obj.name);
  if(i === -1) {
    arr.push(obj)
  } else {
    arr.splice(i, 1);
